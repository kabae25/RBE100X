This is the readme.txt file for the RBE 100X Final Project (Evan Kaba, 12/17/23, B23)

/----------------------------------------------------------------------------------------/
How to get the robot running:
Go to the start.py file, and create new paths following the conventions outlined inside. There you can declare any blocked nodes per path, as well as the destination and source nodes. 

Once complete, upload the code to the XRP, and run the file. Unfortunately I cannot get this to run on startup of the XRP

To test locally, or troubleshoot, run "start.py" in the terminal after commenting out all robot.* calls. This is result in no errors. Executing other files will result in errors, as well as including any XRPlib calls.

/----------------------------------------------------------------------------------------/
Detailed Breakdown of the Codebase theory:
...
Once in the PATH_GEN state, the robot will run dijstra_class.py for path 1, and compile the instructions, which are either turn commands or drive commands. Specifically, there are the following commands: turn_180, swing_left and swing_right (which turn 90 degrees respectively), and fwrd. 

The list of instructions is formatted as the next node to go to. This is beneficial because it provides an ease of comparing where the robot is to where it is trying to go. Because it can tell the direction its to be traveling in, it can relate it's current heading to its future heading and move accordinly. 
...
When the instruction set is complete, if the final path has not been completed, the robot will set it's path_index+=1, and set it's state to PATH_GEN, where it generates the next path's instruction set, and repeat for n number of paths. 

If the final path has been completed, the robot will set it's state to END

In the END state, the robot will print a debug message indicating the program has ended.
